"""
블록체인 덴탈보험 발표자료 PPT v3 생성기
v3 추가 사항 (슬라이드 12~14 신규 삽입):
  - 슬라이드 12: 보험증권(Policy)이란? — 블록체인의 데이터 구조
  - 슬라이드 13: 증권 생성 두 가지 경로 — 직접 생성 vs 청약 심사
  - 슬라이드 14: 컨트랙트 = 준비금 금고 — Reserve Fund on Blockchain
  → 기존 슬라이드 12~31 → 15~34 로 번호 이동
"""
...
def slide_02_toc(prs):
    ...
    items = [
        ...
        ("2부", "시스템 소개",    "3-Layer 아키텍처 / 7가지 핵심 기능 / 보험증권 & 준비금 구조", C_GREEN),
...
def slide_09_problem_vs_solution(prs):
    ...
    add_rect(s, Inches(6.27), Inches(2.5), Inches(0.8), Inches(0.8), fill_color=C_YELLOW)
...
def slide_12_policy_structure(prs):
    """보험증권이란? — 블록체인의 데이터 구조 (v3 신규)"""
    s = blank_slide(prs)
    bg(s)
    title_bar(s, "슬라이드 12  |  보험증권(Policy)이란?",
              "종이 문서가 아닌 블록체인 위의 데이터 구조 — 영구 저장, 변조 불가")

    # 왼쪽: 현실 vs 블록체인 비교
    add_rect(s, Inches(0.3), Inches(1.35), Inches(4.3), Inches(4.85), fill_color=C_BG_CARD)
    add_rect(s, Inches(0.3), Inches(1.35), Inches(4.3), Inches(0.45), fill_color=C_RED)
    add_text(s, "현실의 보험증권", Inches(0.45), Inches(1.38), Inches(4.1), Inches(0.4),
             font_size=Pt(13), bold=True, color=C_BG_DARK)
    add_multiline(s, [
        "종이 또는 PDF 문서",
        "보험사 서버에 저장",
        ...
        "X  '증권이 유효한가?'",
        "X   피보험자가 확인할 방법 없음",
    ], Inches(0.45), Inches(1.9), Inches(4.0), Inches(4.1), font_size=Pt(13), color=C_GRAY)

    # 중앙: 화살표
    add_text(s, "→", Inches(4.75), Inches(3.5), Inches(0.7), Inches(0.6),
             font_size=Pt(30), bold=True, color=C_ACCENT, align=PP_ALIGN.CENTER)

    # 오른쪽: 블록체인 증권 구조체
    add_rect(s, Inches(5.6), Inches(1.35), Inches(7.4), Inches(4.85),
             fill_color=C_BG_CARD, line_color=C_ACCENT, line_width=Pt(1.5))
    add_rect(s, Inches(5.6), Inches(1.35), Inches(7.4), Inches(0.45), fill_color=C_ACCENT)
    add_text(s, "블록체인 보험증권 — Policy 구조체", Inches(5.75), Inches(1.38),
             Inches(7.1), Inches(0.4), font_size=Pt(13), bold=True, color=C_BG_DARK)
    fields = [
        ("id",                 "1",                       "증권 번호 (자동 발급, 영구 고유)"),
        ("patient",            "0x7099...김덴탈 지갑주소", "피보험자 주소 (위조 불가)"),
        ("monthlyPremium",     "50 USDC / 70,000원",      "월 보험료 (계약 시 확정)"),
        ("coverageLimit",      "1,000 USDC",              "보장 한도 (코드로 상한 고정)"),
        ("totalPaid",          "0 → 누적 증가",           "납입 보험료 합산 (자동 집계)"),
        ("maturityDate",       "2027-03-28",              "만기일 (자동 환급 트리거)"),
        ("maturityRefundRate", "70%",                     "만기환급율 (계약 시 확정)"),
        ("active",             "true",                    "증권 유효 여부"),
    ]
    for i, (field, value, comment) in enumerate(fields):
        y = Inches(1.9) + i * Inches(0.44)
        add_text(s, field, Inches(5.75), y, Inches(2.0), Inches(0.38),
                 font_size=Pt(11), color=C_YELLOW)
        add_text(s, value, Inches(7.9), y, Inches(2.1), Inches(0.38),
                 font_size=Pt(11), color=C_ACCENT2)
        add_text(s, comment, Inches(10.1), y, Inches(2.8), Inches(0.38),
                 font_size=Pt(10), color=C_GRAY)

    # 하단 설명
    add_rect(s, Inches(0.3), Inches(6.35), Inches(12.7), Inches(0.65), fill_color=C_BG_CARD2)
    add_text(s, "=> 증권 생성 = 이 구조체가 블록체인에 영구 저장되는 것  |  policyId로 납입·청구·환급 모두 연결",
             Inches(0.5), Inches(6.42), Inches(12.3), Inches(0.5),
             font_size=Pt(13), color=C_YELLOW, align=PP_ALIGN.CENTER)

def slide_13_policy_creation_paths(prs):
    """증권 생성 두 가지 경로 — 직접 생성 vs 청약 심사 (v3 신규)"""
    s = blank_slide(prs)
    bg(s)
    title_bar(s, "슬라이드 13  |  보험증권 생성 두 가지 경로",
              "청약 심사는 증권 생성 이전 단계 — 심사를 통과해야 증권이 발급됩니다")

    # 상단 설명 배너
    add_rect(s, Inches(0.3), Inches(1.35), Inches(12.7), Inches(0.55),
             fill_color=RGBColor(0x05, 0x2E, 0x1A))
    add_text(s, "보험증권은 두 가지 방법으로 생성됩니다. 둘 다 내부적으로 동일한 _createPolicyInternal() 함수를 사용합니다.",
             Inches(0.5), Inches(1.38), Inches(12.3), Inches(0.48),
             font_size=Pt(13), color=C_GREEN, align=PP_ALIGN.CENTER)

    # 경로 A (왼쪽)
    add_rect(s, Inches(0.3), Inches(2.05), Inches(5.9), Inches(3.6),
             fill_color=C_BG_CARD, line_color=C_ORANGE, line_width=Pt(2))
    add_rect(s, Inches(0.3), Inches(2.05), Inches(5.9), Inches(0.45), fill_color=C_ORANGE)
    add_text(s, "경로 A: 관리자 직접 생성 (심사 없음)", Inches(0.45), Inches(2.08),
             Inches(5.7), Inches(0.4), font_size=Pt(13), bold=True, color=C_BG_DARK)
    add_multiline(s, [
        "관리자(보험사)",
        "    createPolicy() 직접 호출",
        "    ↓  onlyOwner — 관리자만 가능",
        ...
        "",
        "X  청약 심사 절차 없음",
    ], Inches(0.5), Inches(2.6), Inches(5.5), Inches(2.8), font_size=Pt(12), color=C_GRAY)

    # VS
    add_text(s, "VS", Inches(6.3), Inches(3.5), Inches(0.75), Inches(0.55),
             font_size=Pt(20), bold=True, color=C_YELLOW, align=PP_ALIGN.CENTER)

    # 경로 B (오른쪽)
    add_rect(s, Inches(7.15), Inches(2.05), Inches(5.9), Inches(3.6),
             fill_color=C_BG_CARD, line_color=C_GREEN, line_width=Pt(2))
    add_rect(s, Inches(7.15), Inches(2.05), Inches(5.9), Inches(0.45), fill_color=C_GREEN)
    add_text(s, "경로 B: 청약 심사 후 승인 (정식 경로)", Inches(7.3), Inches(2.08),
             Inches(5.7), Inches(0.4), font_size=Pt(13), bold=True, color=C_BG_DARK)
    add_multiline(s, [
        "피보험자 → submitApplication()",
        "    ↓  자동 1차 심사",
        "    나이·비율·보유건수 룰 체크",
        "    위험점수(0~100) 자동 산정",
        "    ↓  통과",
        "관리자 → approveApplication()",
        "    ↓  내부 호출",
        "_createPolicyInternal()",
        "    ↓",
        "보험증권 자동 발급",
    ], Inches(7.3), Inches(2.6), Inches(5.5), Inches(2.8), font_size=Pt(12), color=C_ACCENT2)

    # 공통 함수 설명
    add_rect(s, Inches(0.3), Inches(5.75), Inches(12.7), Inches(0.75),
             fill_color=C_BG_CARD2, line_color=C_ACCENT, line_width=Pt(1))
    add_text(s, "공통 내부 함수: _createPolicyInternal()", Inches(0.5), Inches(5.78),
             Inches(5.0), Inches(0.35), font_size=Pt(12), bold=True, color=C_ACCENT)
    add_text(s, "피보험자 주소 / 이름 / 보험료 / 보장한도 / 만기일 / 환급율 → Policy 구조체 생성 → 블록체인 저장 → PolicyCreated 이벤트",
             Inches(5.6), Inches(5.8), Inches(7.3), Inches(0.58),
             font_size=Pt(11), color=C_GRAY)

    add_text(s, "=> 청약 심사(Underwriting)는 증권 생성 이전 단계입니다. 심사 통과 = 증권 발급 조건 충족",
             Inches(0.3), Inches(6.6), Inches(12.7), Inches(0.45),
             font_size=Pt(13), color=C_YELLOW, align=PP_ALIGN.CENTER)

def slide_14_reserve_fund(prs):
    """컨트랙트 = 준비금 금고 (v3 신규)"""
    s = blank_slide(prs)
    bg(s)
    title_bar(s, "슬라이드 14  |  컨트랙트 = 보험 준비금 금고",
              "스마트 컨트랙트 주소가 곧 준비금 계좌 — 코드가 허용한 경우만 출금 가능")

    # 상단: 핵심 메시지
    add_rect(s, Inches(0.3), Inches(1.35), Inches(12.7), Inches(0.55),
             fill_color=RGBColor(0x05, 0x1A, 0x2E))
    add_text(s, "DentalInsurance 컨트랙트 주소 = 회사 준비금이 실제로 보관되는 금고",
             Inches(0.5), Inches(1.38), Inches(12.3), Inches(0.48),
             font_size=Pt(15), bold=True, color=C_ACCENT, align=PP_ALIGN.CENTER)

    # 왼쪽: 자금 흐름도
    add_rect(s, Inches(0.3), Inches(2.05), Inches(6.0), Inches(4.35), fill_color=C_BG_CARD)
    add_rect(s, Inches(0.3), Inches(2.05), Inches(6.0), Inches(0.4), fill_color=C_ACCENT)
    add_text(s, "자금 흐름", Inches(0.45), Inches(2.08), Inches(5.8), Inches(0.35),
             font_size=Pt(13), bold=True, color=C_BG_DARK)

    flows_in = [
        ("입금 경로", C_GREEN),
        ("  관리자 → depositFunds()  준비금 최초 입금 (onlyOwner)", C_GREEN),
        ("  피보험자 → payPremium()  보험료 납입 시 자동 적립", C_GREEN),
        ("  피보험자 → collectPremium()  자동수납 시 자동 적립", C_GREEN),
    ]
    flows_out = [
        ("출금 경로 (코드 조건 충족 시에만)", C_RED),
        ("  → 피보험자  보험금 지급 (오라클 검증 통과)", C_RED),
        ("  → 피보험자  만기환급금 지급 (만기일 도래)", C_RED),
        ("  → 피보험자  약관대출 실행 (한도 이내)", C_RED),
        ("  → 피보험자  약관대출 상환 시 원금 복귀", C_GREEN),
    ]
    y_pos = Inches(2.55)
    for item in flows_in + [("", C_WHITE)] + flows_out:
        text, color = item
        if text == "":
            y_pos += Inches(0.15)
            continue
        add_text(s, text, Inches(0.45), y_pos, Inches(5.7), Inches(0.38),
                 font_size=Pt(11), color=color)
        y_pos += Inches(0.4)

    # 오른쪽: 기존 vs 블록체인 비교
    add_rect(s, Inches(6.55), Inches(2.05), Inches(6.5), Inches(4.35), fill_color=C_BG_CARD)
    add_rect(s, Inches(6.55), Inches(2.05), Inches(6.5), Inches(0.4), fill_color=C_YELLOW)
    add_text(s, "기존 보험사 vs 블록체인 준비금 비교", Inches(6.7), Inches(2.08),
             Inches(6.2), Inches(0.35), font_size=Pt(13), bold=True, color=C_BG_DARK)

    compare_rows = [
        ("항목",             "기존 보험사",           "블록체인 컨트랙트"),
        ("준비금 위치",      "회사 내부 계좌",         "컨트랙트 주소 (공개)"),
        ("잔액 확인",        "보험사만 가능",           "누구나 조회 가능"),
        ("출금 조건",        "보험사 재량",             "코드 조건만 (변경 불가)"),
        ("임의 인출",        "가능",                   "불가 (코드 차단)"),
        ("감사 추적",        "내부 장부",              "블록체인 Tx Hash"),
        ("부도 위험",        "존재",                   "코드가 살아있으면 지급"),
    ]
    for i, (item, old, new_) in enumerate(compare_rows):
        y = Inches(2.55) + i * Inches(0.54)
        row_bg = C_BG_CARD2 if i % 2 == 0 else C_BG_CARD
        if i == 0:
            row_bg = RGBColor(0x22, 0x33, 0x55)
        add_rect(s, Inches(6.55), y, Inches(6.5), Inches(0.5), fill_color=row_bg)
        fc_item  = C_YELLOW if i == 0 else C_GRAY
        fc_old   = C_RED if i > 0 else C_WHITE
        fc_new   = C_GREEN if i > 0 else C_WHITE
        add_text(s, item,  Inches(6.65), y+Inches(0.08), Inches(1.5), Inches(0.35),
                 font_size=Pt(11), color=fc_item, bold=(i==0))
        add_text(s, old,   Inches(8.3),  y+Inches(0.08), Inches(2.0), Inches(0.35),
                 font_size=Pt(11), color=fc_old,  align=PP_ALIGN.CENTER)
        add_text(s, new_,  Inches(10.4), y+Inches(0.08), Inches(2.5), Inches(0.35),
                 font_size=Pt(11), color=fc_new,  align=PP_ALIGN.CENTER)

    # 하단 강조
    add_rect(s, Inches(0.3), Inches(6.5), Inches(12.7), Inches(0.55), fill_color=C_BG_CARD2)
    add_text(s,
             "getContractBalance() 로 실시간 잔액 조회 가능  |  보험사도 코드 밖에서 임의 인출 불가  |  Immutable Trust",
             Inches(0.5), Inches(6.53), Inches(12.3), Inches(0.48),
             font_size=Pt(12), color=C_ACCENT2, align=PP_ALIGN.CENTER)

# ══════════════════════════════════════════════════════════════
# 슬라이드 15~34 (v2의 12~31, 번호만 +3)
# ══════════════════════════════════════════════════════════════

def slide_15_underwriting(prs):
    feature_slide(prs, 15, "기능 ①: 청약 심사 (Underwriting Automation)",
        ...
        ["피보험자: 이름, 나이, 보험료, 보장한도 입력",
         ...
         "위험점수 자동 산정 (0~100점)",
         "        ↓",
         "관리자 최종 승인 → 보험증권 자동 발급 (슬라이드 13 참조)"],
        ["Immutable Trust:",
         "심사 기준이 코드로 공개",
         "→ 자의적 차별·거절 불가",
         "",
...
def slide_16_premium(prs):
    feature_slide(prs, 16, "기능 ②: 보험료 자동수납 (Zero-Operation Collection)",
        "Approve 1회 → 이후 매월 자동 이체",
        ["최초 1회: [자동납부 ON] → approve 서명",
         "  '보험 컨트랙트에게 이체 권한 부여'",
         "  납입금은 컨트랙트(준비금 금고)로 자동 적립",
         "",
...
def slide_17_claim(prs):
    feature_slide(prs, 17, "기능 ③: 보험금 청구 (Real-time Settlement)",
...
def slide_18_sequence(prs):
    s = blank_slide(prs)
    bg(s)
    title_bar(s, "슬라이드 18  |  보험금 청구 시퀀스 다이어그램",
    ...
    steps = [
        ...
        ("5", "최종 지급",          "검증 통과 → 컨트랙트(준비금 금고)에서 지갑으로 즉시 이체 + 기록",   C_GREEN),
    ]
    y_start = Inches(1.4)
...
def slide_19_hira(prs):
    feature_slide(prs, 19, "기능 ④: HIRA API 연동 (Enterprise-Ready Integration)",
...
def slide_20_maturity(prs):
    feature_slide(prs, 20, "기능 ⑤: 만기환급 자동 지급 (Automated Maturity Refund)",
...
def slide_21_loan(prs):
    feature_slide(prs, 21, "기능 ⑥: 약관대출 (Instant Policy Loan)",
...
def slide_22_currency(prs):
    feature_slide(prs, 22, "기능 ⑦: 이중 통화 (Multi-Currency Liquidity)",
...
def slide_23_metamask(prs):
    feature_slide(prs, 23, "연동 방식 ①: MetaMask 지갑 연동",
...
def slide_24_contract_call(prs):
    single_slide(prs, 24, "연동 방식 ②: 스마트 컨트랙트 호출 흐름",
        "보험료 납부 예시 — 내부 동작 4단계",
        ["'보험료 납부' 버튼 클릭 시 내부 동작:",
         ...
         "   ① 증권 유효성 및 활성 상태 확인",
         "   ② Stablecoin.transferFrom(피보험자 → 컨트랙트(준비금 금고), $50)",
...
def slide_25_oracle(prs):
    feature_slide(prs, 25, "연동 방식 ③: Oracle Node 연동",
        ...
        ["블록체인의 한계:",
         ...
         "oracleVerifyAndProcess() 호출",
         "        ↓",
         "승인 → 준비금 금고에서 즉시 지급 / 거절 → 사유 + Tx Hash 기록"],
        ["데이터 무결성:",
         "블록체인 신뢰성 +",
         "외부 API 실용성",
         "동시 활용",
...
def slide_26_events(prs):
    single_slide(prs, 26, "연동 방식 ④: 이벤트 기반 자동화 (Zero-Operation)",
...
def slide_27_security(prs):
    s = blank_slide(prs)
    bg(s)
    title_bar(s, "슬라이드 27  |  보안 설계 — Immutable Trust 구현",
...
def slide_28_full_flow(prs):
    single_slide(prs, 28, "전체 보험 플로우 정리", "가입부터 만기까지 — 사람 개입 최소화",
        ["[청약 신청]  submitApplication()",
         "  → 자동 심사 (Immutable Rules: 연령/비율/보유건수)",
         "  → 관리자 승인 1회 → 보험증권 자동 발급 (_createPolicyInternal)",
         "",
         "[보험료 납부]  payPremium() / collectPremium() (자동)",
         "  → 매월 납입 → 준비금 금고(컨트랙트)에 적립 → 총납입액 블록체인 기록",
         "",
         "[보험금 청구]  submitClaim()",
         "  → Oracle Node 자동 검증 → 준비금 금고에서 Real-time Settlement (수초)",
         "  → 결과 + Transaction Hash → 블록체인 영구 기록",
         "",
         "[약관대출]  requestPolicyLoan() → repayPolicyLoan()",
         "  → 준비금 금고에서 Instant Policy Loan 즉시 지급 / 원금+이자 상환",
         "",
         "[만기환급]  processMaturityRefund() 자동",
         "  → 준비금 금고에서 총납입액 × 70% → 피보험자 지갑 자동 이체",
         "",
         "=> 사람 개입: 최초 가입 승인 1회뿐  |  Zero-Operation  |  24/7 자동 동작"])

def slide_29_demo_usdc(prs):
    feature_slide(prs, 29, "데모 ①: USDC — Real-time Settlement 시연",
        "청약 → 납부 → 청구 → 즉시 지급 전체 플로우",
        ["1. 관리자(Account #0)로 접속",
         "   컨트랙트 잔액 $50,000 확인 (준비금 금고)",
         "",
         "2. 박청약(35세) 청약 승인",
...
def slide_30_demo_reject(prs):
    s = blank_slide(prs)
    bg(s)
    title_bar(s, "슬라이드 30  |  데모 ②: 거절 시나리오 — 시스템 정교함 증명",
...
def slide_31_demo_krw(prs):
    feature_slide(prs, 31, "데모 ③: KRW 원화 모드 (Multi-Currency Liquidity)",
...
def slide_32_business_impact(prs):
    s = blank_slide(prs)
    bg(s)
    title_bar(s, "슬라이드 32  |  비즈니스 임팩트 & 향후 로드맵",
...
def slide_33_compare(prs):
    s = blank_slide(prs)
    bg(s)
    title_bar(s, "슬라이드 33  |  강점 요약: 기존 보험 vs 블록체인 보험",
...
def slide_34_closing(prs):
    ...
    add_text(s, '조건이 충족되면 준비금 금고에서 자동으로 지급된다."',
...
def main():
    ...
    # ── 1부: 기초 개념 ──────────────────────────────────────────
    slide_01_cover(prs)               # 01: 표지
    slide_02_toc(prs)                 # 02: 목차
    slide_03_pain(prs)                # 03: 현행 보험의 한계
    slide_04_vision(prs)              # 04: 비전 — Code-as-Contract
    slide_05_blockchain(prs)          # 05: 블록체인이란?
    slide_06_contract(prs)            # 06: 스마트 컨트랙트
    slide_07_address(prs)             # 07: 주소 개념
    slide_08_token(prs)               # 08: 토큰 (Stablecoin)
    slide_09_problem_vs_solution(prs) # 09: 기존 vs 블록체인 비교
    slide_10_architecture(prs)        # 10: 3-Layer 아키텍처

    # ── 2부: 시스템 소개 ────────────────────────────────────────
    slide_11_features_grid(prs)       # 11: 7가지 핵심 기능 그리드

    # ▶ v3 신규 슬라이드 (보험증권·준비금 개념 설명) ──────────────
    slide_12_policy_structure(prs)    # 12: [NEW] 보험증권이란? 블록체인 구조체
    slide_13_policy_creation_paths(prs) # 13: [NEW] 증권 생성 두 가지 경로
    slide_14_reserve_fund(prs)        # 14: [NEW] 컨트랙트 = 준비금 금고

    # ── 기능 상세 (v2 대비 번호 +3) ────────────────────────────
    slide_15_underwriting(prs)        # 15: 기능① 청약심사
    slide_16_premium(prs)             # 16: 기능② 보험료 자동수납
    slide_17_claim(prs)               # 17: 기능③ Real-time Settlement
    slide_18_sequence(prs)            # 18: 청구 시퀀스 다이어그램
    slide_19_hira(prs)                # 19: 기능④ HIRA API
    slide_20_maturity(prs)            # 20: 기능⑤ 만기환급
    slide_21_loan(prs)                # 21: 기능⑥ 약관대출
    slide_22_currency(prs)            # 22: 기능⑦ 이중통화

    # ── 3부: 연동 방식 ──────────────────────────────────────────
    slide_23_metamask(prs)            # 23: MetaMask 연동
    slide_24_contract_call(prs)       # 24: 컨트랙트 호출 흐름
    slide_25_oracle(prs)              # 25: Oracle Node 연동
    slide_26_events(prs)              # 26: 이벤트 기반 자동화
    slide_27_security(prs)            # 27: 보안 설계
    slide_28_full_flow(prs)           # 28: 전체 플로우 정리

    # ── 4부: 데모 ───────────────────────────────────────────────
    slide_29_demo_usdc(prs)           # 29: 데모① USDC
    slide_30_demo_reject(prs)         # 30: 데모② 거절 시나리오
    slide_31_demo_krw(prs)            # 31: 데모③ KRW

    # ── 5부: 비즈니스 가치 ──────────────────────────────────────
    slide_32_business_impact(prs)     # 32: 비즈니스 임팩트 & 로드맵
    slide_33_compare(prs)             # 33: 강점 비교표 (준비금 항목 추가)
    slide_34_closing(prs)             # 34: 마무리

    out = r"C:\test_bl1\블록체인_덴탈보험_발표자료_v3.pptx"
    prs.save(out)
    print(f"[OK] v3 저장 완료: {out}")
    print(f"[OK] 총 슬라이드: {len(prs.slides)}장")
    print()
    print("[v3 NEW] slide 12: Policy 구조체")
    print("[v3 NEW] slide 13: 증권 생성 두 경로")
    print("[v3 NEW] slide 14: 컨트랙트=준비금 금고")
